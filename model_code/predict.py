...
Usage:
  ...
  python predict.py --text "text" --model saved_model_roberta_te
"""
import argparse
import os
import sys
...
DEFAULT_MODEL = os.path.join(os.path.dirname(__file__), 'saved_model_roberta_hx')
...
def load_model(model_dir):
    if not os.path.isdir(model_dir):
        ...
        print("Please run step3_twitter_roberta_finetune.py first to train and save the model.",
              file=sys.stderr)
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description='Hate Speech Classifier using fine-tuned Twitter-RoBERTa')
    ...
    parser.add_argument('--model', type=str, default=DEFAULT_MODEL,
                        help='Path to saved model directory (default: saved_model_roberta_hx)')
