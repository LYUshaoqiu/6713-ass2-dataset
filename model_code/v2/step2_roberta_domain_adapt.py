"""
Domain Adaptation: fine-tune saved_model_roberta_hx on course dataset.
Strategy: start from the already-trained HateXplain RoBERTa model, continue
training on course data with a small LR to avoid catastrophic forgetting.
RoBERTa has no token_type_ids and requires attn_implementation='eager'.
"""
import os, gc
...
# ── Paths ─────────────────────────────────────────────────
BASE_MODEL   = 'saved_model_roberta_hx'        # 已训练好的 HateXplain RoBERTa 模型
...
SAVE_DIR     = 'saved_model_roberta_hx_course'  # 领域适应后保存
...
class HateDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # RoBERTa does NOT produce token_type_ids
...
print('\n' + '='*60)
print('STEP 1: Baseline (saved_model_roberta_hx on course test)')
print('='*60)
...
plt.tight_layout()
plt.savefig('course_domain_adapt_roberta_cm.png', dpi=150); plt.close()
print('Saved: course_domain_adapt_roberta_cm.png')
...
comparison.to_csv('course_domain_adapt_roberta_results.csv', index=False)
print('\nSaved: course_domain_adapt_roberta_results.csv')
