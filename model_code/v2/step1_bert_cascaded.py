...
course_df = pd.read_csv(COURSE_DIR + 'course_reviews_cleaned.csv').rename(columns={'comment': 'text'})
...
course_train, course_val = train_test_split(
    course_train_val, test_size=0.176, random_state=42, stratify=course_train_val['label'])
# 0.176 ≈ 15/85, so final split ≈ 70/15/15
...
def get_class_weights(labels, n_classes):
    """Balanced class weights as a tensor."""
...
def evaluate_model(model, loader):
    """Evaluate with unweighted loss for honest validation metrics."""
    model.eval()
...
def predict_cascaded(texts, s1_state, s2_state):
    ...
    Final output: 0=Normal, 1=Offensive, 2=Hate
    """
    texts = list(texts)
    dummy = [0] * len(texts)
    ...
    s1_loader = DataLoader(HateDataset(texts, dummy),
                           batch_size=BATCH_SIZE, num_workers=0)
    s1_preds = []
    with torch.no_grad():
    ...
    # ── Merge: 0=Normal, 1=Offensive, 2=Hate ──
    final_preds = []
...
def eval_and_save(s1_state, s2_state, test_df, dataset_name, save_s1, save_s2):
    ...
    plt.tight_layout()
    fname = f'bert_cascaded_{dataset_name.lower()}_cm.png'
...
def run_pipeline(train_df, val_df, test_df, dataset_name, prefix):
    ...
    dist = mixed_train['label'].value_counts().sort_index()
    for lbl, cnt in dist.items():
        print(f'    {LABELS[lbl]}: {cnt} ({cnt/len(mixed_train)*100:.1f}%)')
    ...
    s1_state = run_finetune(s1_train, s1_val,
                            tag=f'{dataset_name}/Stage1',
                            num_labels=2, cw=s1_cw)
    ...
    s2_state = run_finetune(s2_train, s2_val,
                            tag=f'{dataset_name}/Stage2',
                            num_labels=2, cw=s2_cw)

    # ── Evaluate on main test set ──
    result_main = eval_and_save(
        s1_state, s2_state, test_df, dataset_name,
        save_s1=f'saved_model_bert_{prefix}_stage1',
        save_s2=f'saved_model_bert_{prefix}_stage2')
    ...
    result_course = eval_and_save(
        ...
        save_s1=f'saved_model_bert_{prefix}_stage1',   # already saved, overwrite ok
        save_s2=f'saved_model_bert_{prefix}_stage2')
...
results_df.to_csv('bert_cascaded_results.csv', index=False)
print('\nSaved: bert_cascaded_results.csv')
