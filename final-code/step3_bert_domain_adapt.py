"""
Domain Adaptation: BERT-base (HateXPlain) → further adapt on course dataset.
Base model: models/bert_HateXPlain/  (kept intact, NOT overwritten)
DA model:   models/bert_da_course/
...
Prerequisites: run step1_finetune_all.py first so models/bert_HateXPlain/ exists.

Outputs (saved to results/):
  results/bert_da_course_cm.png        — CM: DA model on course test
  results/bert_da_hatexplain_cm.png    — CM: DA model on HateXPlain test
  results/bert_da_tweeteval_cm.png     — CM: DA model on TweetEval test
  results/bert_da_history.csv          — training history (all LRs)
  results/bert_da_results.csv          — full comparison table (3 test sets)
  models/bert_da_course/               — domain-adapted checkpoint
"""
import os, gc
...
def eval_on_testset(model, test_df, test_name, cm_path):
    ...
    plt.title(f'BERT-HateXPlain + DA\n{test_name}')
...
baseline_rows = [
    eval_on_testset(base_model, hx_test,     'HateXPlain', 'results/bert_base_hx_cm.png'),
    eval_on_testset(base_model, te_test,      'TweetEval',  'results/bert_base_te_cm.png'),
    eval_on_testset(base_model, course_test,  'Course',     'results/bert_base_course_cm.png'),
]
...
pd.DataFrame(all_history).to_csv('results/bert_da_history.csv', index=False)
...
da_rows = [
    eval_on_testset(adapted_model, hx_test,    'HateXPlain', 'results/bert_da_hatexplain_cm.png'),
    eval_on_testset(adapted_model, te_test,     'TweetEval',  'results/bert_da_tweeteval_cm.png'),
...
for row in baseline_rows: row['Stage'] = 'Before DA (bert_HateXPlain)'
...
print('\nSaved: results/bert_da_results.csv')
