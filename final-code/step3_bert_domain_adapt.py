...
# Remove rows with some miss text or label
course_df.dropna(subset=['text', 'label'], inplace=True)

#remove row index after cleaning
...
def eval_on_testset(model, test_df, test_name, confusion_matrix_path):
    ...
    plt.tight_layout()
    plt.savefig(confusion_matrix_path, dpi=150); plt.close()


    # Save the confusion matrix image for this test set
    print(f'  CM saved: {confusion_matrix_path}')
...
baseline_model_results = [
    eval_on_testset(base_model, hatexplain_test_df,     'HateXPlain', 'results/bert_base_hx_cm.png'),
    eval_on_testset(base_model, tweeteval_test_df,      'TweetEval',  'results/bert_base_te_cm.png'),
