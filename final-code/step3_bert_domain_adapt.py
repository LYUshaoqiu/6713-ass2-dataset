"""
Domain Adaptation: BERT-base fine-tuned on HateXPlain → further adapt on course dataset.
Strategy: start from the already-trained HateXPlain model (models/bert_HateXPlain/),
...
Outputs (saved to results/ and models/):
  results/bert_da_course_cm.png         — confusion matrix on course test set
  results/bert_da_results.csv           — before vs after comparison
  models/bert_da_course/                — domain-adapted model checkpoint
"""
import os, gc
...
# ── Paths ──────────────────────────────────────────────────
BASE_MODEL   = 'models/bert_HateXPlain'    # output of step1_finetune_all.py
...
print('\n' + '='*60)
print('STEP 1: Baseline (bert_HateXPlain on course test set)')
print('='*60)
...
BATCH_SIZE    = 8
EPOCHS        = 5   # more epochs since course dataset is small
...
hist_df.to_csv('results/bert_da_history.csv', index=False)
...
plt.tight_layout()
plt.savefig('results/bert_da_course_cm.png', dpi=150); plt.close()
print('Saved: results/bert_da_course_cm.png')
...
comparison.to_csv('results/bert_da_results.csv', index=False)
